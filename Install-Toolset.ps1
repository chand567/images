...
foreach ($tool in $tools) {
    Write-Host "Tool: $($tool | ConvertTo-Json -Depth 10)"
    ...
    foreach ($toolVersion in $tool.versions) {
        ...
        | Where-Object { ($_.arch -eq $tool.arch) -and ($_.platform -eq $tool.platform) -and ($_.platform_version -eq $tool.platform_version)} `
