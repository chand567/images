...
Describe "istioctl" {
    $testCases = @("/opt/.istioctl/bin/istioctl") | ForEach-Object { @{istioctlCommand = $_} }

    $permissions = (ls -la /opt/.istioctl/bin/istioctl)
    Write-Host "Permissions of istioctl file /opt/.istioctl/bin/istioctl : $permissions"

    Write-Host "Total Test Cases are : $testCases"

    It "<istioctlCommand>" -TestCases $testCases {
        param (
            [string] $istioctlCommand
        )

        "$istioctlCommand version" | Should -ReturnZeroExitCode
    }
}
