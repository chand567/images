...
installOpenJDK() {
    local JAVA_VERSION=$1
    echo "Attempting to install Temurin JDK version ${JAVA_VERSION}..."
    # Check if the package exists before attempting to install
    if ! apt-cache show "temurin-${JAVA_VERSION}-jdk" > /dev/null 2>&1; then
        echo "Error: Temurin JDK version ${JAVA_VERSION} is not available in the repository."
        exit 1
    fi 
    # Install the specified version
    retry_command "apt-get -y install temurin-${JAVA_VERSION}-jdk"
    ...
    if [[ ! -d "$javaVersionPath" ]]; then
        echo "Error: Installation directory ${javaVersionPath} does not exist."
        exit 1
    fi
...
# Install Ant
retry_command "apt-get install -y --no-install-recommends ant ant-optional"
...
retry_command "download_with_retries ${mavenDownloadUrl} '/tmp' 'maven.zip'"
...
retry_command "download_with_retries ${gradleDownloadUrl} '/tmp' 'gradleLatest.zip'"
